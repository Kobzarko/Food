...
function calc() {
  ...
  function initLocalSettings(selector, activeClass) {
    ...
    elements.forEach(elem => {
      elem.classList.remove(activeClass); //если элемент содержит атрибут id который равен значению из localStorage
  ...
  initLocalSettings(".calculating__choose_big div", "calculating__choose-item_active"); // изменяет данные каждый раз когда меняется значение в каком то поле
    ...
    elements.forEach(elem => {
      elem.addEventListener("click", e => {
        ...
        if (e.target.getAttribute("data-ratio")) {
          ratio = +e.target.getAttribute("data-ratio"); //запись данных в localStorage

          localStorage.setItem("ratio", ratio);
        } else {
          sex = e.target.getAttribute("id"); //запись данных в localStorage
        ...
        console.log(ratio, sex); // убираем класс активности для всех элементов
  ...
  getStaticInfo(".calculating__choose_big div", "calculating__choose-item_active"); // обрабатывает каждый отдельный инпут

  function getDynamicInfo(selector) {
    const input = document.querySelector(selector); // запись данных в определенную переменную
...
function cookieConsent() {
  ...
  const storageType = localStorage;
  const consentPropertyType = "site_consent"; // получаем либо true либо false
  // true не показывать окно
  // false показать окно

  const hasConsented = () => storageType.getItem(consentPropertyType) === "true" ? true : false; // сохраняем согласие пользователя если true
...
function forms(formSelector, modalTimerId) {
  ...
  function bindpostData(form) {
    form.addEventListener("submit", e => {
      e.preventDefault();
      let statusMessage = document.createElement("img"); // напрямую обращаемся к атрибуту

      statusMessage.src = message.loading; // добавляем стили нашему img
      ...
      const formData = new FormData(form); // formData.entries возвращает массив собственных перечисляемых свойств
      // Object.fromEntries() преобразует список пар ключ-значение в объект.

      const json = JSON.stringify(Object.fromEntries(formData.entries())); // // перебрать formdata в объект
      // const object = {};
  ...
  function showThanksModal(message) {
    ...
    (0,_modal__WEBPACK_IMPORTED_MODULE_0__.openModal)(".modal", modalTimerId); // создать элемент

    const thanksModal = document.createElement("div"); // добавить класс элементу, класс старого окна

    thanksModal.classList.add("modal__dialog"); // создать окно
    ...
    document.querySelector(".modal").append(thanksModal); // удалить новое окно через 4 сек

    setTimeout(() => {
      thanksModal.remove(); // показываем старое окно

      prevModalDialog.classList.add("show");
      prevModalDialog.classList.remove("hide"); // прячем блок .modal со старым окном
...
function modal(triggerSelector, modalSelector, modalTimerId) {
  const modalTrigger = document.querySelectorAll(triggerSelector),
        modal = document.querySelector(modalSelector); // чтобы не вызывать сразу функцию openModal оборачиваем ее в стрелочную функцию
...
function slider({
  ...
  const prev = document.querySelector(prevArrow),
        ...
        width = window.getComputedStyle(slidesWrapper).width; //Window.getComputedStyle()  возвращает объект, содержащий значения всех CSS-свойств элемента,
  //полученных после применения всех активных таблиц стилей

  let slideIndex = 1; // на сколько отступили вправо или влево

  let offset = 0; // нумерация
  ...
  slidesField.style.width = 100 * slides.length + "%"; // ограничить показ слайдов до одного

  slidesWrapper.style.overflow = "hidden"; // разместить слайды горизонтально

  slidesField.style.display = "flex"; // сместить за полсекунды

  slidesField.style.transition = "0.5s all"; // каждому слайду устанавливаем одинаковую ширину

  slides.forEach(slide => {
    slide.style.width = width;
  }); // убираем абсолютное позиционирование у элементов внутри слайдера

  slider.style.position = "relative"; // создать обертку для точек
  ...
  next.addEventListener("click", function () {
    ...
    slidesField.style.transform = `translateX(-${offset}px)`; //контролируем slideIndex дошли до конца =1
    ...
    setOpacity(); // dots.forEach((dot) => (dot.style.opacity = ".5"));
    // dots[slideIndex - 1].style.opacity = 1;
  });
  ...
  dots.forEach(dot => {
    dot.addEventListener("click", e => {
      ...
      slideIndex = slideTo;
      offset = delNotDigits(width) * (slideTo - 1); //смещаем наш блок

      slidesField.style.transform = `translateX(-${offset}px)`; // добавить ноль
...
const getResource = async url => {
  const res = await fetch(url); // .ok - состояние запроса
...
/* harmony import */ var _modules_cookieConsent__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ./modules/cookieConsent */ "./js/modules/cookieConsent.js");
...
window.addEventListener("DOMContentLoaded", function () {
  ...
  const modalTimerId = setTimeout(() => (0,_modules_modal__WEBPACK_IMPORTED_MODULE_2__.openModal)(".modal", modalTimerId), 300000); // вызов функций
  ...
  (0,_modules_cookieConsent__WEBPACK_IMPORTED_MODULE_8__.default)();
});
})();
